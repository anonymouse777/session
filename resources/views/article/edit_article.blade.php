...
@section ('pageBody')
	...
	<div class="article_form_width">
		...
		<form method="post" action="/home/edit/{{ $article->id }}">
			@csrf
			<div class="form-group">
				<input type="text" name="title" class="form-control" value="{{ $article->title }}" required>
			</div>
			<div class="form-group">
				<textarea name="paragraph" placeholder="Enter a Paragraph" rows="7" class="form-control" required>{{ $article->paragraph }}</textarea>
			</div>
			<div class="form-group">
